...
client.request("Observation?" + query, {
  ...
  function(ob) {
    ...
    //var weight = getWeightAndHeightValue(byCodes('54567-3'), '3141-9');
    //var height = getWeightAndHeightValue(byCodes('54567-3'), '3137-7');
...
function addWeightAnnotation() {
  ...
  //displayAnnotation(annotation);
    weight[0].note = [{
      "authorString": client.user.fhirUser,
      "time": getDateTime(),
      "text": annotation
  }]
...
//get current date time with required format
function getDateTime(){
    ...
    var timeZoneFirst = currentdatetime.getTimezoneOffset()+ 1;
    var timeZoneLast = currentdatetime.getTimezoneOffset()%60;
    ...
    if(timeZoneFirst.toString().length == 1){
        timeZoneFirst = '0'+timeZoneFirst;
    }
    if(timeZoneLast.toString().length == 1){
        timeZoneLast = '0'+timeZoneLast;
    }

    return year+'-'+month+'-'+date+'T'+hour+':'+minute+':'+second+'+'+timeZoneFirst+':'+timeZoneLast;
}
